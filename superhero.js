	...
	.then( (data) => {
		...
		let item = `<div class="card item h-50 m-4 hero" style="width: 13rem;">
						...
						<div class="card-body d-flex flex-row justify-content-around">
							<h5 class="card-title text-center"><a class="text-decoration-none" href=heroDetails.html?id=${data.id}>${data.name}</a></h5>							    
							<div class="fav-btn" >
								<img class="favImg" id="${data.id}"  src="https://encrypted-tbn1.gstatic.com/images?q=tbn:ANd9GcTN9L9sZEFl1O22EqaziUINo8s_wvY4ZAjyIHwU0MOefGmJECRc" alt="Unfav" >
							</div>
						</div>
					</div>`
...
async function addToFavourite(e){
	...
	e.target.src="https://encrypted-tbn1.gstatic.com/images?q=tbn:ANd9GcRQ2u2sXWiI8Cc5SZGOyygZvsi74Sypwpw5cKZvcNQwkIc3hAhh"
...
async function removeFromFavourite(e){
	...
	e.target.src="https://encrypted-tbn1.gstatic.com/images?q=tbn:ANd9GcTN9L9sZEFl1O22EqaziUINo8s_wvY4ZAjyIHwU0MOefGmJECRc"
	e.target.alt="Unfav"
...
async function showFavExistence(f){
	...
	if(fav.includes(id)){
		...
		//update fav icon
		child.setAttribute('src','https://encrypted-tbn1.gstatic.com/images?q=tbn:ANd9GcRQ2u2sXWiI8Cc5SZGOyygZvsi74Sypwpw5cKZvcNQwkIc3hAhh')
		...
		.then( (data) => {
			if(data.response === 'success'){
				...
				for( hero  of heroes ){
					...
					a.setAttribute('href',`heroDetails.html?id=${hero.id}`)
